    ...
    return productsAmount ? (
      <>
        {Object.entries(products).map((product) => {
          const item = [pizza1, pizza2].filter(
            (pizza) => pizza.id.toString() === product[0]
          )[0];
          const amount = product[1] as number;
          return (
            <CartItem
              itemAmount={amount}
              id={item.id || 1}
  ...
  return (
    <>
      <div className="container">
        ...
        >
          {[pizza1, pizza2, pizza3, pizza4, pizza1, pizza2, pizza3, pizza4].map(
            ({ title, price, imgSrc, description, id }) => (
              <ProductCard
                title={title}
                price={price}
                imgSrc={imgSrc}
