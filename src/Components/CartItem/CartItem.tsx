...
const CartItem = ({ id, price, itemAmount }: CartItemProps) => {
  console.log(price, itemAmount);
  ...
  const incrementAmount = () => {
    setAmount(amount + 1);
    const orders = localStorage.getItem('orders') + ',' + id;
  ...
  const decrementAmount = () => {
    ...
    if (orders) {
      const idIndex = orders.indexOf(id.toString());
      ...
      <div className={styles.itemBox}>
        ...
        <div className={styles.itemBox__calc}>
          <div className={styles.itemBox__calc_total}>
            <span>{formatter.format(price * amount * 1000)} сум</span>
          </div>
